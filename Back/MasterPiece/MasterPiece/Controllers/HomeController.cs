...
namespace MasterPiece.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            var products = db.Products.OrderByDescending(p => p.Product_id).
                Where(x=>x.Store.isAccepted==true&&x.isDeleted!=true&&x.Store.isBlocked!=true&&x.Quantity>0).ToList();
        ...
        {
            var products = db.Products.Where(x=>x.Store.isBlocked!=true&& x.isDeleted !=true && x.Quantity > 0).ToList();
        ...
        public ActionResult CheckOutItems() 
        {
            ...
            message.Subject = "Your New Order-NoCats";
   ...
   + $"<tr>\r\n            <td align=\"left\" bgcolor=\"#ffffff\" style=\"padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;\">\r\n              <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\">\r\n                <tr>\r\n                  <td align=\"left\" bgcolor=\"#EDEDED\" width=\"75%\" style=\"padding: 12px;font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;\"><strong>Order #</strong></td>\r\n                  <td align=\"left\" bgcolor=\"#EDEDED\" width=\"25%\" style=\"padding: 12px;font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;\"><strong>{guid.ToString()}</strong></td>\r\n                </tr>";
        ...
        public ActionResult SingleCategory(int? id)
        {
            ...
            var Products = db.Products.Where(x=>x.Category_id==id&&x.isDeleted!=true&& x.Store.isAccepted == true && x.isDeleted != true && x.Store.isBlocked != true && x.Quantity > 0).ToList();
            return View(Products);
        }
        public ActionResult SingleProduct(int? id)
        {
            ...
            if (Product.isDeleted == true || Product.Store.isBlocked == true) { return View("Error"); }
        ...
        public ActionResult AddItem(int id, string returnUrl)
        {
            ...
            if (!itemFound && items.Count > 0) 
            {

                int productId = Convert.ToInt32(items[0].Split('|')[0].Split('_')[0]);
                int storeId = Convert.ToInt32(db.Products.Find(productId).Store_id);
                int newProductStoreId = Convert.ToInt32(db.Products.Find(id).Store_id);
                if (newProductStoreId != storeId)
                {
        ...
        public ActionResult SingleStore(int? id)
        {
            ...
            if (store.isBlocked==true || store.isAccepted !=true)
            {
                return View("Error");
            }
            var Products = db.Products.Where(x => x.Store_id == id &&x.isDeleted!=true && x.Store.isAccepted == true && x.isDeleted != true && x.Store.isBlocked != true && x.Quantity > 0).ToList();
            return View(Products);
        }
